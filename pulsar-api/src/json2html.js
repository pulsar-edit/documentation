...
function convert(name, content, version) {
  ...
  for (const section of content.sections) {
    file += `<h3 class="api__section-name" id="${anchorize(section.name)}">${section.name}</h3>`;
...
function lookupNullSections(content, env, hasOtherSections) {
  ...
  file += `<h3 data-count="${totalCount}" class="api__section-name" id="${anchorize(sectionName)}">${sectionName}</h3>`;
